 ...
 * @author jchen
 */
public class MarkLogicRecord implements Writable {
	public static final Log LOG =
	    LogFactory.getLog(MarkLogicRecord.class);
	
	private Node node;
	
	public MarkLogicRecord() {}
	
	public MarkLogicRecord(Node node) {
		this.node = node;
	}
	
	static DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
	
	public MarkLogicRecord(ResultItem item) {
		ItemType type =	item.getItemType();
		LOG.info("item type: " + item.getItemType());
		LOG.info("item value: " + item.asString());
	    try {
